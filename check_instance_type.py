...
parser = ArgumentParser(description="This script will check instance types")
parser.add_argument("instanceType",default="t3.micro", help="Instance type")
parser.add_argument("--vpcId", "-v", help="VPC ID")
instanceType = parser.parse_args().instanceType.strip()
vpc_id = parser.parse_args().vpcId
	...
	try:
		response = ec2_client.describe_vpcs(
			Filters=[
				...
				# In my account I had delete all my default VPCs for security reasons 
				#if you want to sort it as per default VPC enable below filter
				# {
				# 'Name': 'is-default',
				# 'Values': [
				# 	'Yes',
...
def get_instance_types (instance_type):
...
def describe_intance(instance_type,vpc_id):
	try:
		...
		print("=================================================")
		print(f"| Name Tag \t\t| Instance ID \t\t|\n=================================================")		
		for i in response['Reservations']:
			for r in i['Instances']:
				instance_id = r['InstanceId']
				for x in r['Tags']:
					instance_name_tag = x['Value']
					print(f"| {instance_name_tag} \t| {instance_id} \t|")
...
def cli(instanceType,vpc_id):
	if vpc_id == None:
		vpc_id = get_default_vpcs()	
	print(f"\nVPC ID : {vpc_id}\nInstance Type : {instanceType}\n")
	describe_intance(instanceType,vpc_id)

if __name__ == "__main__":
	cli(instanceType,vpc_id)